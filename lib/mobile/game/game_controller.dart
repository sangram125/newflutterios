...
class GameController extends GetxController {
  ...
  final String url =
      'https://page.api.hungama.com/v1/page/game/partnerGames?storeId=1&affiliateId=1003';
  Dio dio = Dio();
  RxList<GameData> games = <GameData>[].obs;
  ...
  fetchData({Key? key, required FetchRows mediaDetailFuture}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Assuming the API returns a list of games
        games.value = (response.data as List)
            .map((gameJson) => GameData.fromJson(gameJson))
            .toList();
        print(games[0].title); // Example usage
      } else {
