import 'dart:async';
import 'package:dor_companion/mobile/screens/fullscreen_toast.dart';
...
class SearchViewPage extends se.SearchDelegate {
  ...
  SearchViewPage({
    ...
    searchFieldLabel: "Search for comedy",
  );
  ...
  Widget buildResults(BuildContext context) {
    final searchController = Get.put(SearchViewController());
    if (query.isEmpty) {
      return _buildSuggestions(searchController.searchHistory, context);
   ...
   _onQueryChanged1(BuildContext context) {
    ...
    return FutureBuilder<SearchSuggestionResult>(
      ...
      builder: (context, snapshot) {
        switch (snapshot.connectionState) {
          ...
          default:
            if (snapshot.hasData) {
              return _buildSuggestions(snapshot.data!.results, context);
            } else {
              return Container();
            }
        }
